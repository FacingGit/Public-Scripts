if IS_LOADED and not _G.IS_DEBUG == true then
	-- error("Infinite Sky is already running!",0)
	return
end
...
if not game:IsLoaded() then
	...
	notLoaded.Text = 'Infinite Sky is waiting for the game to load'
	game.Loaded:Wait()
	notLoaded:Destroy()
end
...
Title.TextSize = 18
Title.Text = "Infinite Sky FE v" .. currentVersion
...
About.TextSize = 14
About.Text = "Plugins are .IS files and should be located in the 'workspace' folder of your exploit."
...
PluginsHint.Text = "Download plugins from the IS Discord (discord.io/Infinitesky)"
...
ISMouse = Players.LocalPlayer:GetMouse()
...
reference = (function()
	local main = create({
		...
		{114,"TextLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Font=3,Name="Text",Parent={112},Position=UDim2.new(0,8,0,30),Size=UDim2.new(1,-8,0,32),Text="You can join the Discord server to get support with IS,  and read up on more documentation such as the Plugin API.",TextColor3=Color3.new(1,1,1),TextSize=14,TextWrapped=true,TextXAlignment=0,ZIndex=10,}},
	...
	inviteButton.MouseButton1Click:Connect(function()
		...
		if func then
			func("https://discord.io/Infinitesky")
		...
		inviteButton.Text = "Copy Discord Invite Link (https://discord.io/Infinitesky)"
	end)
	dragGUI(main)
	main.Parent = PARENT
...
function saves()
	if writefileExploit() then
		if pcall(function() readfile("IS_FE.IS") end) then
			if readfile("IS_FE.IS") ~= nil then
				local success, response = pcall(function()
					local json = HttpService:JSONDecode(readfile("IS_FE.IS"))
					...
					if json.keepIS ~= nil then KeepInfYield = json.keepIS else KeepInfYield = true end
				...
				if not success then
					...
					writefileCooldown("IS_FE.IS", defaults)
					wait()
					saves()
				end
			else
...
function updatesaves()
	if nosaves == false and writefileExploit() then
		...
		writefileCooldown("IS_FE.IS", HttpService:JSONEncode(update))
	end
end
...
ISMouse.KeyDown:Connect(function(Key)
	if (Key==prefix) then
		Cmdbar:CaptureFocus()
		spawn(function()
...
SaveChatlogs.MouseButton1Down:Connect(function()
	if writefileExploit() then
		if #scroll_2:GetChildren() > 0 then
			...
			local writelogs = '-- Infinite Sky Chat logs for "'..placeName..'"\n'
			for _, child in pairs(scroll_2:GetChildren()) do
				writelogs = writelogs..'\n'..child.Text
			end
...
function selectPart()
	...
	local function HighlightPart()
		if selected.Adornee ~= ISMouse.Target then
			selectionBox.Adornee = ISMouse.Target
		else
			selectionBox.Adornee = nil
		end
	end
	ActivateHighlight = ISMouse.Move:Connect(HighlightPart)
	local function SelectPart()
		if ISMouse.Target ~= nil then
			selected.Adornee = ISMouse.Target
			Path.Text = getHierarchy(ISMouse.Target)
		end
	end
	ClickSelect = ISMouse.Button1Down:Connect(SelectPart)
end
...
CMDs = {}
CMDs[#CMDs + 1] = {NAME = 'Cheats', DESC = 'Sky Cheats // Loads personal Cheats'}
CMDs[#CMDs + 1] = {NAME = 'chatlogger', DESC = 'Sky Cheats // Loads ChatLogger'}
CMDs[#CMDs + 1] = {NAME = 'pianoplayer / 
', DESC = 'Sky Cheats // Auto Play Piano'}
CMDs[#CMDs + 1] = {NAME = 'DetectFling /Detectfl', DESC = 'Sky Cheats // Detects and Protects from people flinging'}
CMDs[#CMDs + 1] = {NAME = 'Nameless / NoName', DESC = 'Sky Cheats // Gives No Name Hide Name'}
CMDs[#CMDs + 1] = {NAME = 'Hitbox', DESC = 'Sky Cheats // Hitbox Extender'}
...
CMDs[#CMDs + 1] = {NAME = 'hideIS', DESC = 'Hides the main IS GUI'}
CMDs[#CMDs + 1] = {NAME = 'showIS / unhideIS', DESC = 'Shows IS again'}
CMDs[#CMDs + 1] = {NAME = 'keepIS', DESC = 'Auto execute IS when you teleport through servers'}
CMDs[#CMDs + 1] = {NAME = 'unkeepIS', DESC = 'Disable keepIS'}
CMDs[#CMDs + 1] = {NAME = 'togglekeepIS', DESC = 'Toggle keepIS'}
...
function checkTT()
	local t
	local guisAtPosition = COREGUI:GetGuiObjectsAtPosition(ISMouse.X, ISMouse.Y)
	...
	if t ~= nil and t:GetAttribute("Title") ~= nil then
		...
		local xP
		local yP
		if ISMouse.X > 200 then
			xP = x - 201
		else
			xP = x + 21
		end
		if ISMouse.Y > (ISMouse.ViewSizeY-96) then
			yP = y - 97
		else
			yP = y
		end
			...
			if cmd then
				...
				if infTimes then
					while lastBreakTime < cmdStartTime do
						...
						if not success and _G.IS_DEBUG then
...
local MousePositionToVector2 = function()
	return Vector2.new(ISMouse.X, ISMouse.Y)
end
...
local function clicktpFunc()
	pcall(function()
		...
		getRoot(Players.LocalPlayer.Character).CFrame = ISMouse.Hit + Vector3.new(0,7,0)
	end)
end

ISMouse.Button1Down:Connect(function()
	for i,v in pairs(binds) do
		...
		elseif v.COMMAND == 'clickdel' then
			...
			if input == 'RightClick' and UserInputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton2) then
				pcall(function() ISMouse.Target:Destroy() end)
...
function addPlugin(name)
	if name:lower() == 'plugin file name' or name:lower() == 'IS_fe.IS' or name == 'IS_fe' then
		notify('Plugin Error','Please enter a valid plugin')
	else
		local file
		local fileName
		if name:sub(-3) == '.IS' then
			pcall(function() file = readfile(name) end)
			fileName = name
		else
			pcall(function() file = readfile(name..'.IS') end)
			fileName = name..'.IS'
		end
		if file then
...
Players.LocalPlayer.OnTeleport:Connect(function(State)
	if State == Enum.TeleportState.Started then
		if KeepInfYield and queueteleport then
			queueteleport("loadstring(game:HttpGet('https://raw.githubusercontent.com/EdgeIS/Infinitesky/master/source'))()")
		end
	end
end)
...
addcmd('keepIS', {}, function(args, speaker)
...
addcmd('unkeepIS', {}, function(args, speaker)
	if queueteleport then
		KeepInfYield = false
		updatesaves()
	else
...
addcmd('toggleunkeepIS', {}, function(args, speaker)
	if queueteleport then
...
function sFLY(vfly)
	...
	flyKeyDown = ISMouse.KeyDown:Connect(function(KEY)
		if KEY:lower() == 'w' then
			CONTROL.F = (vfly and vehicleflyspeed or ISflyspeed)
		elseif KEY:lower() == 's' then
			CONTROL.B = - (vfly and vehicleflyspeed or ISflyspeed)
		elseif KEY:lower() == 'a' then
			CONTROL.L = - (vfly and vehicleflyspeed or ISflyspeed)
		elseif KEY:lower() == 'd' then 
			CONTROL.R = (vfly and vehicleflyspeed or ISflyspeed)
		elseif QEfly and KEY:lower() == 'e' then
			CONTROL.Q = (vfly and vehicleflyspeed or ISflyspeed)*2
		elseif QEfly and KEY:lower() == 'q' then
			CONTROL.E = -(vfly and vehicleflyspeed or ISflyspeed)*2
		end
		pcall(function() workspace.CurrentCamera.CameraType = Enum.CameraType.Track end)
	end)
	flyKeyUp = ISMouse.KeyUp:Connect(function(KEY)
		if KEY:lower() == 'w' then
			CONTROL.F = 0
		elseif KEY:lower() == 's' then
			CONTROL.B = 0
...
addcmd('float', {'platform'},function(args, speaker)
	...
	if pchar and not pchar:FindFirstChild(floatName) then
		task.spawn(function()
			...
			qUp = ISMouse.KeyUp:Connect(function(KEY)
				if KEY == 'q' then
					FloatValue = FloatValue + 0.5
				end
			end)
			eUp = ISMouse.KeyUp:Connect(function(KEY)
				if KEY == 'e' then
					FloatValue = FloatValue - 0.5
				end
			end)
			qDown = ISMouse.KeyDown:Connect(function(KEY)
				if KEY == 'q' then
					FloatValue = FloatValue - 0.5
				end
			end)
			eDown = ISMouse.KeyDown:Connect(function(KEY)
				if KEY == 'e' then
					FloatValue = FloatValue + 0.5
				end
			end)
...
function deleteGuisAtPos()
	pcall(function()
		local guisAtPosition = Players.LocalPlayer.PlayerGui:GetGuiObjectsAtPosition(ISMouse.X, ISMouse.Y)
		for _, gui in pairs(guisAtPosition) do
...
addcmd('hideIS',{},function(args, speaker)
	isHidden = true
	...
	minimizeNum = 0
	minimizeHolder()
	if not (args[1] and tostring(args[1]) == 'nonotify') then notify('IS Hidden','You can press the prefix key to access the command bar') end
end)

addcmd('showIS',{'unhideIS'},function(args, speaker)
	isHidden = false
	minimizeNum = -20
	if wasStayOpen then
		maximizeHolder()
...
--Detect Invis ppl
addcmd('DetectFling',{'detectfl'},function(args, speaker)
...
--------
addcmd('Nameless',{'noname'},function(args, speaker)
	loadstring(game:GetObjects("rbxassetid://5688101100")[1].Source)()
end)
--Hitbox Extender GUI
addcmd('Hitbox',{'hitbox'},function(args, speaker)
...
addcmd('chatlogger',{'chatlog'},function(args, speaker)
	notify("Loading",'Hold on a sec')
	-- Full credit to Sky, creator of GUI
	loadstring(game:HttpGet('https://pastebin.com/raw/wCDw9F7r',true))()
	end)


	addcmd('Cheats',{'Gui'},function(args, speaker)
		notify("Loading",'Hold on a sec')
		-- Full credit to Sky, creator of GUI
		nouni=false
	local function lo(l)
	...
	--------------------------GAME SCRIPTS CREATED BY SKY1234---------------------------
	if game.PlaceId==5774201912 then 
	lo("https://pastebin.com/raw/qAR8f8aE") --Paranomica
	end
	
	if game.PlaceId==3311165597 then 
		lo("https://aizen.ml/uploads/dbu.lua") --Dragon Ball Ultimate
		end
	
		if game.PlaceId==734159876 then 
			lo("https://raw.githubusercontent.com/LOOF-sys/Roblox-Shit/main/SharkBite.lua") --Sharkbite
			end
	
			if game.PlaceId==1735775055 then 
				lo("https://raw.githubusercontent.com/boop71/cappuccino-new/main/loader.lua") --Iron Man Simulator // Cappuccino Hub
				end
	
				if game.PlaceId==6097258548 then 
					lo("https://raw.githubusercontent.com/boop71/cappuccino-new/main/loader.lua") --Iron Man Simulator 2 // Cappuccino Hub
					end

						if game.PlaceId==3527629287 then 
							lo("https://raw.githubusercontent.com/radjahfromdiscord/iNEXT/main/source") --Big Paintball
							end
	
							if game.PlaceId==893973440 then 
								lo("https://pastebin.com/raw/b76uANY7") --BABFT
								end
	
								if game.PlaceId==3725149043 then 
									lo("https://pastebin.com/6sYgBbW9") --Super Doomspire Brick Battle
									end
	
									if game.PlaceId==893973440 then 
										lo("https://raw.githubusercontent.com/1201for/V.G-Hub/main/V.Ghub") --Flee the Facility
										end
	
										if game.PlaceId==155615604 then 
											lo("https://raw.githubusercontent.com/FacingGit/Public-Scripts/main/1oDbrLv40E1SH") --Prison Life // SKY'S SCRIPT HUB
											end
...
addcmd('tptool', {'teleporttool'}, function(args, speaker)
	...
	TpTool.Activated:Connect(function()
		...
		HRP.CFrame = CFrame.new(ISMouse.Hit.X, ISMouse.Hit.Y + 3, ISMouse.Hit.Z, select(4, HRP.CFrame:components()))
	end)
end)
...
addcmd('hovername',{},function(args, speaker)
	...
	nbUpdateFunc = ISMouse.Move:Connect(updateNameBox)
end)
...
ISMouse.Move:Connect(checkTT)

task.spawn(function()
	local success, latestVersionInfo = pcall(function() 
		local versionJson = game:HttpGet('https://raw.githubusercontent.com/EdgeIS/Infinitesky/master/version')
